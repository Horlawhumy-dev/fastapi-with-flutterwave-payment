...
@user_router.post('/debit', status_code=status.HTTP_200_OK, tags=['User Balance Debit'])
async def debit_user_balance(json_data: dict, db: Session = Depends(get_db)):
    return UserService.debit_user(json_data["email"], json_data["amount"], db)
